...
export function initFeedback() {
  ...
  $("submit-feedback-btn").addEventListener("click", submitFeedbackData);
}

function updateStars(rating) {
  document.querySelectorAll("#feedback-stars .star").forEach((star) => {
...
async function submitFeedbackData() {
  ...
  const feedback = {
    ...
    username: appState.currentUserData?.name || "Anonymous",
  ...
  try {
    await addDoc(collection(appState.db, "feedback"), feedback);
